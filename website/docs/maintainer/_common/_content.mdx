...
**@xpack-dev-tools/windows-build-tools** is an xpm package that packages platform-specific archives, built with the [xPack Build Box (XBB)](https://xpack.github.io/xpack-build-box/).
...
(such as **4.4.1-4.1**) with the following expression:
...
To download the pre-release archive for the specific platform
...
tagged such as **v4.4.1-4** (mind the dash in the middle!) and
named such as **xPack Windows Build Tools v4.4.1-4** (mind the dash),
...
- **Commit** all changes with the message
  _**package.json: update URLs for 4.4.1-4.1 release**_
...
- Navigate to the [GitHub Releases](https://github.com/xpack-dev-tools/windows-build-tools-xpack/releases) page
...
npm --prefix ~/Work/xpack-dev-tools/windows-build-tools-xpack.git/build-assets install && \
xpm install -C ~/Work/xpack-dev-tools/windows-build-tools-xpack.git/build-assets && \
xpm run test-release -C ~/Work/xpack-dev-tools/windows-build-tools-xpack.git/build-assets
```
