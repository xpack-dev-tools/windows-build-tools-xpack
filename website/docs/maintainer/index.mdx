...
title: How to make new xPack Windows Build Tools releases
...
named like **xPack Windows Build Tools v{customField('xpackVersion')}** (mind the dash),
...
npm dist-tag add @xpack-dev-tools/windows-build-tools@${customField('xpackVersion')}.${customField('npmSubversion')} latest
npm dist-tag ls @xpack-dev-tools/windows-build-tools
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`npm unpublish @xpack-dev-tools/windows-build-tools@${customField('xpackVersion')}.${customField('npmSubversion')}
`} </CodeBlock>
