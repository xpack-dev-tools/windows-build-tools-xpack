...
title: How to install windows-build-tools
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/windows-build-tools@${customField('version')} --verbose
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/AppData/Roaming/xPacks/\@pack-dev-tools/windows-build-tools/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
- <code>xpack-windows-build-tools-{customField('xpackVersion')}-win32-x64.zip</code>
...
example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\windows-build-tools\\xpack-windows-build-tools-{customField('xpackVersion')}</code>.
